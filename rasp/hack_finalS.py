...
try:
	while True:
		if GPIO.input(fire)==0:
			print "FIRE"
			payload = {'receive':True, 'longitude':'37.469433', 'latitude':'126.955342'}
			r=requests.post(Fireurl,json=payload)
			print "fine"
		else:
			payload = {'receive':False, 'longitude':'37.469433', 'latitude':'126.955342'}
			r=requests.post(Fireurl,json=payload)
		...
		if GPIO.input(button2)==0:
			print "EARTH"
			payload = {'receive':True, 'longitude':'37.469433', 'latitude':'126.955342' ,'intensity':'8.0'}
			r=requests.post(Earthurl,json=payload)
			print "fine"
		else:
			payload = {'receive':False, 'longitude':'37.469433', 'latitude':'126.955342','intensity':'0.0'}
			r=requests.post(Earthurl,json=payload)
